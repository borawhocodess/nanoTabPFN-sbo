...
parser.add_argument(
    "-priordump",
    type=str,
    default="other/dumps/50x3_3_100k_classification.h5",
    help="path to the prior dump",
)
parser.add_argument(
    "-saveweights",
    type=str,
    default="other/model/nanotabpfn_classifier_weights.pth",
...
def epoch_callback(epoch, epoch_time, mean_loss, model):
    ...
    print(
        f"epoch {epoch:5d} | time {epoch_time:5.2f}s | mean loss {mean_loss:5.2f} | avg accuracy {avg_score:.3f}",
        flush=True,
    )
